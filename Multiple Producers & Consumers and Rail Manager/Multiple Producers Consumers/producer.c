...
void acquireMutexLock(int sub_sema_index)
{
	...
	if(sub_sema_index == QUEUE_0_MUTEX || sub_sema_index == QUEUE_1_MUTEX)
			sem_operator.sem_flg = IPC_NOWAIT; // non-blocking wait, if mutex already acquired
	else sem_operator.sem_flg = 0; // blocking wait, if mutex already acquired
	
	if(semop(semID,&sem_operator,1) == -1)
			checknext = 1;
